...
def get_all_file_names(path, clean_text=None, clean_path=False, required_text=None, declined_text=None, name_parser=None):
...
def get_all_file_names_contains(path,  required_text=None):
    files = glob.glob(f"{path}*")
...
def get_base_paths(base_path, folder_name, folder_path=os.path.realpath(__file__)):
    ...
    create_folder(base_media_path)
    create_folder(base_data_path)
