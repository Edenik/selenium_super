...
def get_time_stamp(date:datetime=None, time_splitter=':', date_splitter='-', between_splitter=' ', show_seconds=False):
    seconds_text = ''
    if show_seconds:
...
def get_file_time_stamp(date_only=False, hour_only=False, date=None):
    ts =  get_time_stamp(time_splitter="-",between_splitter="_", date=date)
